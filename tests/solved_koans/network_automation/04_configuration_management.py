"""
Network Koan 4: Configuration Management - Finding the Backdoor (MEDIUM-HARD) - SOLVED
==============================================================================
...
assert len(original_lines) == 29, "Fix this: how many non-empty lines in original?"
assert len(current_lines) == 44, "Fix this: how many non-empty lines in current?"

# 🔍 CLUE: Current config has MORE lines (attacker added things!)
lines_added = len(current_lines) - len(original_lines)
assert lines_added == 15, "Fix this: how many lines were added?"
...
assert len(added_lines) == 11, "Fix this: how many unique lines were added?"
...
assert len(original_vlans) == 3, "Fix this: how many VLANs in original?"
assert len(current_vlans) == 4, "Fix this: how many VLANs in current?"

# Find the rogue VLAN
rogue_vlan = [v for v in current_vlans if v not in original_vlans]
assert len(rogue_vlan) == 1, "Fix this: how many unauthorized VLANs?"
assert rogue_vlan[0] == 999, "Fix this: what is the unauthorized VLAN number?"
...
assert len(original_users) == 1, "Fix this: how many users in original?"
assert len(current_users) == 2, "Fix this: how many users in current?"

# Find backdoor account
backdoor_users = [u for u in current_users if u not in original_users]
assert len(backdoor_users) == 1, "Fix this: how many unauthorized users?"
assert backdoor_users[0] == "admin2", "Fix this: what is the backdoor username?"
...
assert len(original_snmp) == 2, "Fix this: how many SNMP communities originally?"
assert len(current_snmp) == 3, "Fix this: how many SNMP communities now?"

# Find added SNMP community (backdoor access!)
added_snmp = [c for c in current_snmp if c not in original_snmp]
assert len(added_snmp) == 1, "Fix this: how many unauthorized SNMP communities?"
assert added_snmp[0] == "secret123", "Fix this: what is the backdoor SNMP community string?"
...
assert len(gi024_original) == 0, "Fix this: was Gi0/24 in original config? (0 means no)"
...
assert unauthorized_vlans == 1, "Fix this: how many unauthorized VLANs found?"

unauthorized_users = analyzer.analyze_users()
assert unauthorized_users == 1, "Fix this: how many unauthorized users found?"

unauthorized_snmp = analyzer.analyze_snmp()
assert unauthorized_snmp == 1, "Fix this: how many unauthorized SNMP communities?"

# Check findings
total_findings = analyzer.get_finding_count()
assert total_findings == 3, "Fix this: total security findings? (3 total)"

critical_findings = analyzer.get_critical_findings()
assert len(critical_findings) == 1, "Fix this: how many CRITICAL findings?"
assert critical_findings[0]["type"] == "UNAUTHORIZED_USER", "Fix this: what type is the critical finding?"
...
assert v1.version == 1, "Fix this: what is v1 version number?"
assert v1.line_count < v2.line_count, "Fix this: v2 should have more lines than v1"

# Calculate changes
config_growth = v2.line_count - v1.line_count
assert config_growth == 15, "Fix this: how many lines added in v2?"

print("""
✓ Network Koan 4 completed!
