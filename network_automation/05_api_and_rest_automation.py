"""
Network Koan 5: API and REST Automation - Tracing the Attack Vector (HARD)
===========================================================================
...
assert len(api_logs) == __, "Fix this: how many API calls were made?"

# All requests came from the same IP
attacker_ips = [log["source_ip"] for log in api_logs]
unique_ips = set(attacker_ips)
assert len(unique_ips) == __, "Fix this: how many unique source IPs?"
assert list(unique_ips)[0] == __, "Fix this: what is the attacker's IP?"
...
assert len(successful_requests) == __, "Fix this: how many successful API calls?"
...
assert len(post_requests) == __, "Fix this: how many POST requests?"

put_requests = [log for log in api_logs if log["method"] == "PUT"]
assert len(put_requests) == __, "Fix this: how many PUT requests?"
...
assert len(created_resources) == __, "Fix this: how many resources were created?"

# Check what was created
vlan_creation = created_resources[0]
assert vlan_creation["data"]["vlan_id"] == __, "Fix this: what VLAN was created?"

user_creation = created_resources[1]
assert user_creation["data"]["username"] == __, "Fix this: what username was created?"
...
assert vlans_before["status"] == __, "Fix this: what is the status code?"
assert len(vlans_before["data"]) == __, "Fix this: how many VLANs before attack?"

# Attack Step 1: Create malicious VLAN
response1 = device.create_vlan(999, "HIDDEN")
assert response1["status"] == __, "Fix this: what status code for creating VLAN?"
assert response1["message"] == __, "Fix this: what is the success message?"

# After attack - verify VLAN was added
vlans_after = device.get_vlans()
assert len(vlans_after["data"]) == __, "Fix this: how many VLANs after creating 999?"
...
assert response2["status"] == __, "Fix this: status code for user creation?"
assert len(device.users) == __, "Fix this: how many users after creating admin2?"
...
assert response3["status"] == __, "Fix this: status code for interface update?"

# Verify the interface configuration
gi024 = device.get_interface("GigabitEthernet0/24")
assert gi024["status"] == __, "Fix this: status for getting interface?"
assert gi024["data"]["vlan"] == __, "Fix this: what VLAN is Gi0/24 configured for?"
...
assert response["status"] == __, "Fix this: status code with valid admin token?"
...
assert response["status"] == __, "Fix this: status code with read-only token? (403=Forbidden)"

# Test with invalid token
response = secure_api.create_vlan_authenticated("invalid_token", 100, "NewVLAN")
assert response["status"] == __, "Fix this: status code with invalid token? (401=Unauthorized)"
...
assert auth_check["authenticated"] == __, "Fix this: is the stolen token valid?"
assert auth_check["user_info"]["role"] == __, "Fix this: what role does the stolen token have?"
...
assert len(attack_timeline) == __, "Fix this: how many events in timeline?"
assert attack_timeline[0]["time"] == __, "Fix this: what time was the first API call?"
...
assert len(failed_requests) == __, "Fix this: how many failed requests? (all succeeded)"
...
assert len(cleanup_script) == __, "Fix this: how many cleanup API calls needed?"
...
assert results[0] == __, "Fix this: first request status?"
assert results[1] == __, "Fix this: second request status?"
assert results[2] == __, "Fix this: third request status?"

# 4th and 5th should be rate limited
assert results[3] == __, "Fix this: fourth request status? (429=Too Many Requests)"
assert results[4] == __, "Fix this: fifth request status?"

# Count how many were blocked
blocked = [r for r in results if r == 429]
assert len(blocked) == __, "Fix this: how many requests were rate limited?"
...
assert len(attack_steps) == __, "Fix this: how many steps in the attack script?"

# This matches our API logs!
assert attack_steps[0]["call"] == __, "Fix this: what was the first API call?"
assert attack_steps[1]["payload"]["username"] == __, "Fix this: username created in step 2?"

print("✓ Network Koan 5 completed!")
print("""
