"""
Network Koan 6: Network Testing and Validation - Securing the Network (HARD)
=============================================================================
...
test1.run()
assert test1.passed == __, "Fix this: should clean config pass?"
...
test2.run()
assert test2.passed == __, "Fix this: should compromised config pass?"
...
assert test3.passed == __, "Fix this: should test with admin2 pass?"
...
test4.run()
assert test4.passed == __, "Fix this: should cleaned users pass?"
...
test5.run()
assert test5.passed == __, "Fix this: should weak passwords pass?"
...
test6.run()
assert test6.passed == __, "Fix this: should strong passwords pass?"
...
test7.run()
assert test7.passed == __, "Fix this: should all devices be reachable?"

# Test with offline device
devices_with_offline = ["Switch-01", "Switch-02", "Switch-03"]
test8 = ConnectivityTest(devices_with_offline)
test8.run()
assert test8.passed == __, "Fix this: should test pass with offline device?"
...
test9.run()
assert test9.passed == __, "Fix this: should secure config pass?"
...
test10.run()
assert test10.passed == __, "Fix this: should insecure config pass?"
...
test11.run()
assert test11.passed == __, "Fix this: should backdoor interface pass?"
...
test12.run()
assert test12.passed == __, "Fix this: should clean interfaces pass?"
...
assert len(results) == __, "Fix this: how many tests in the suite?"

# Get summary
summary = suite.get_summary()
assert summary["total"] == __, "Fix this: total number of tests?"
assert summary["passed"] == __, "Fix this: how many tests passed? (all 6)"
assert summary["failed"] == __, "Fix this: how many tests failed?"
assert summary["pass_rate"] == __, "Fix this: what is the pass rate percentage?"

# All tests should pass
assert suite.all_passed() == __, "Fix this: did all tests pass?"
...
assert compromised_summary["passed"] == __, "Fix this: how many passed on compromised? (0)"
assert compromised_summary["failed"] == __, "Fix this: how many failed on compromised? (3)"
assert compromised_suite.all_passed() == __, "Fix this: did compromised network pass all tests?"
...
assert monitor.should_alert(scan1) == __, "Fix this: should we alert for secure network?"

# Scan 2: Network is compromised
scan2 = monitor.perform_scan(compromised_suite)
assert monitor.should_alert(scan2) == __, "Fix this: should we alert for compromised network?"

assert monitor.get_scan_count() == __, "Fix this: how many scans performed?"
...
print("✓ Network Koan 6 completed!")
print("""
